...
export function BottomNav() {
  ...
  return (
    <div className="sticky bottom-0 left-0 right-0 z-[100] px-4 pt-2 pb-[calc(1.5rem+env(safe-area-inset-bottom))] bg-gradient-to-t from-background via-background/80 to-transparent pointer-events-none sm:pb-6">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center gap-1 py-2.5 px-3 rounded-2xl transition-all duration-300 ${
                isActive 
